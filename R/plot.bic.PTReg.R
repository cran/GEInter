...
#' \code{"bic.PTReg"} object.
#'
#' @param x Fitted \code{"bic.PTReg"} model.
#' @param \dots Other graphical parameters to plot.
#' @return A heat map for estimated coefficients.
#' @seealso \code{bic.PTReg}, and \code{predict}, and \code{coef}
#' methods.
...
#'
#' @method plot bic.PTReg
#' @import ggplot2
#' @importFrom reshape2 melt
#' @importFrom graphics plot
#' @export
#' @export plot.bic.PTReg
plot.bic.PTReg=function(x,...){
  object=x
  alpha=c(0,object$alpha_estimate)
  q=length(alpha)-1
  beta=t(object$beta_estimate)
  p=length(beta[,1])
